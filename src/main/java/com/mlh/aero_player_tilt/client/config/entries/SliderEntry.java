...
import com.mlh.aero_player_tilt.client.config.ui.SnapSlider;
...
public class SliderEntry extends ConfigOptionList.Entry {
    ...
    private java.util.function.DoubleFunction<String> readout;
    ...
    public SliderEntry(String labelKey, String tooltipKey,
        ...
        this(labelKey, tooltipKey, config, sliderMin, sliderMax, step, hardMin, hardMax, false);
    }
    ...
    private void buildWidgets() {
        slider = new SnapSlider(0, 0, 100, 20,
                ...
                v -> {
                    if (syncingFromBox || locked) return;
        ...
        editBox.setResponder(text -> {
            if (syncingFromSlider || locked) return;
            try {
                double parsed = isInt
    ...
    public SliderEntry withReadout(java.util.function.DoubleFunction<String> readout) {
    ...
    public void render(GuiGraphics gfx, int index, int top, int left, int width, int height,
        ...
        if (readout != null) {
            String caption = readout.apply(currentValue);
            if (caption != null && !caption.isEmpty()) {
                net.minecraft.client.gui.Font font = Minecraft.getInstance().font;
                gfx.drawString(font, caption,
                        slX - gap - font.width(caption), top + (ENTRY_H - font.lineHeight) / 2 + 1,
                        0xFF9A9A9A, false);
            }
        }
